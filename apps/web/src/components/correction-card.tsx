...
export function CorrectionCard({
  ...
  const isDone = correction.status === "accepted" || correction.status === "rejected";
  ...
  return (
    <button
      ...
      className={cn(
        ...
        correction.status === "accepted" && "opacity-60",
        correction.status === "rejected" && "opacity-40"
      )}
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Change description */}
          {(correction.original_value || correction.corrected_value) && !isEditing && (
              ...
              {correction.corrected_value && (
                <span className="font-mono text-brand-400">
                  {correction.status === "rejected"
                    ? correction.corrected_value
                    : editValue || correction.corrected_value}
                </span>
              )}
            </div>
          )}
            ...
            <div className="mt-2 flex gap-1.5" onClick={(e) => e.stopPropagation()}>
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setIsEditing(false);
                  setEditValue(correction.corrected_value ?? "");
                }}
              >
                <X className="size-3" />
              </Button>
          ...
          {/* Action buttons */}
          {correction.status === "pending" && !isEditing && (
            <div
