...
def evaluate_images(slide: Slide, brand: BrandRuleset) -> list[Issue]:
    ...
    min_dpi = (
        _DEFAULT_MIN_DPI if tolerances.image_min_dpi is None
        else tolerances.image_min_dpi
    )
    ar_tolerance = (
        _DEFAULT_ASPECT_RATIO_TOLERANCE
        if tolerances.aspect_ratio_tolerance is None
        else tolerances.aspect_ratio_tolerance
    )
    issues: list[Issue] = []
